...
OpenRouter is a separate provider with its own client, key and settings, even when `OPENROUTER_MODEL` names a DeepSeek model. It is a gateway: OpenRouter chooses, per call, which upstream provider hosting that model serves it. `OpenRouterLlmClient` takes the DeepSeek client's approach to structured output — JSON mode, the schema in the system prompt, local validation as the check — because OpenRouter decides support for a `json_schema` constraint per upstream endpoint. Reasoning is switched off with OpenRouter's own `reasoning: {"effort": "none"}`, never DeepSeek's `thinking` field, and the temperature is 0. Every request also sets `provider: {"require_parameters": true, "data_collection": "deny"}`: only upstream endpoints supporting every parameter sent, and none that may store the data. One difference in error handling is structural: once an upstream provider has accepted a call, OpenRouter reports a later failure as an HTTP 200 whose body is an `error` object with no `choices`, so the client checks for that before reading a reply, and maps it — like an HTTP error — by its numeric code alone, never its message or metadata. The read uses the same single deadline and reply-size ceiling as DeepSeek (`OPENROUTER_TIMEOUT_SECONDS`).
