...
- Ranked-list endpoint returning score, band, must-have coverage, and warning flags. ✅ `GET /api/jobs/{job_id}/ranking`. The warning codes are `MUST_HAVE_NOT_EVIDENCED`, `SCORE_UNDEFINED`, `EVIDENCE_DOWNGRADED`, `INSTRUCTION_LIKE_TEXT_IN_CV` and `MULTI_COLUMN_LAYOUT` — all drawn from data earlier stages already stored, and none of them changes a score or a position. The last two are where a parse-time finding — an injection attempt, an unreliable reading order — finally reaches a human.
